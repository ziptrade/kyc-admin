module JWTAuthentication
  class Decoder
    ...
    def decode(auth_token)
      decoded_token = JWT.decode auth_token, hmac_secret, true,
                                       {:verify_iat => true,
                                        :iss => issuer,
                                        :verify_iss => true,
                                        :leeway => leeway,
      ...
      private_data = decoded_token.detect {|dict| dict.has_key? 'data'}
    ...
    def valid?(private_data)
      private_data && private_data['data'] && private_data['data'].has_key?('user_auth_token')
    end
